...
request =
  LOAD '$input'
  USING st.happy_camper.pig.scala.aggregate.storage.CombinedLogLoader;

html_request =
  FILTER request
  BY
        method == 'GET'
    AND request_path matches '(?:/[^ ]*)?/(?:[^/]+\\.html)?'
    AND (status_code == 200 OR status_code == 304);
...
fixed_html_request = 
  ...
  GENERATE 
    ...
    st.happy_camper.pig.scala.aggregate.evaluation.LongToDateFormat(requested_time, 'yyyy/MM/dd') AS request_date;
